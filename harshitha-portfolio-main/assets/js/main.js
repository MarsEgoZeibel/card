...
const typeText = () => {
    ...
    home__title.textContent += str[position];
    position++;
    setTimeout(typeText, getRandomInt())
}

function getRandomInt(min =50,max =550){
...
(function () {
    var square = document.querySelector('.about__img');
    ...
    var observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        if (entry.isIntersecting) {
          entry.target.classList.add('about__img-animation');
        }
      });
    });
